...
public class MainWindow extends FrameWindow implements ActionListener {
    ...
    public MainWindow(String title) {
        ...
        primeThread = new PrimeObservableThread();//subject 객체를 생성함.

        textFieldWindow = new TextFieldWindow(TEXTFIELD_WINDOW_TITLE, X, Y + HEIGHT + GAP, WIDTH, HEIGHT,primeThread);//내가 생성자를 추가함

        labelWindow = new LabelWindow(LABEL_WINDOW_TITLE, X, Y + (HEIGHT + GAP) * 2, WIDTH, HEIGHT,primeThread);//생성자를 통해서 의존성 주
    ...
    public JPanel createPanel(int width, int height) {//gui생성하는 부분
    ...
    public void actionPerformed(ActionEvent e) {
        if (e.getSource() == updateTextFieldObserverButton) {
            ...
            if(textFieldWindow.getObserverState()){

                textFieldWindow.removeObserverSelf();
                updateTextFieldObserverButton.setText(TEXTFIELD_OBSERVER_BUTTON_TITLE);

            }else{
                textFieldWindow.registerObserverSelf();
            ...
            if(labelWindow.getObserverState()){

                labelWindow.removeObserverSelf();
                updateLabelObserverButton.setText(LABEL_OBSERVER_BUTTON_TITLE);

            }else{
                labelWindow.registerObserverSelf();
